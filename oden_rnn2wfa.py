...
def make_params():
    ...
    params += [("regr", {
        "comment": f"regr{m}dist" + str(bfs) + str(sbd),
        "eps": eps,
        "max_length": 20,
        "eta": 0.1,
        "gamma": 0.1,
        "cap_m": m,
        "depth_eager_search": -1,
        "regressor_maker_name": x,
        "experimental_constant_allowance": True,
        "experimental_automatic_eta": True,
        "experimental_disable_restarting": True,
        "experimental_bfs": bfs,
        "experimental_sort_by_dist": sbd,
        "experimental_disable_equivalent": e,
        "experimental_skip_existing": False,
        "experimental_halt_long_word": 20,
        "experimental_wfa_only": True
    }) for x in reg for e in experimental_disable_equivalent for bfs, sbd in bfsopt for m in [2, 5] for eps in [0.05]]

    # quits = [500, 1000, 2000, 3000, 5000]
    # params += [("search", {
    #     "comment": "search" + str(q),
    #     "e": 0.05,
    #     "quit_number": q,
    #     "experimental_reset": True,
    #     "experimental_disable_equivalent": ede,
    #     "experimental_skip_existing": False,
    #     "experimental_halt_long_word": 20
    # }) for ede in experimental_disable_equivalent for q in quits]
    return params


# Returns the list of the tasks
def make_tasks_artificial():
    #     rnns = """20190521160227_hualalai_masaki_1-1.zip
    # hualalai_masaki_2-2.zip
    # hualalai_masaki_3-4.zip
    # hualalai_masaki_4-5.zip"""
#     rnns = """20190514171225_rnn_no3_abcdefghijklmno_10_3.zip
...
#biased
    rnns = """20190827130614-172-31-29-75_rnn_no1_abcd_10_3.zip
20190827130150-172-31-17-80_rnn_no1_abcdef_10_3.zip
20190827110535-172-31-17-80_rnn_no1_abcdefghij_10_3.zip
20190827110716-172-31-29-75_rnn_no1_abcdefghij_15_3.zip
20190827110218-172-31-23-42_rnn_no1_abcdefghij_20_3.zip
20190827110441-172-31-28-104_rnn_no1_abcdefghijklmno_10_3.zip
20190827110040-172-31-20-198_rnn_no1_abcdefghijklmno_15_3.zip
20190827105956-172-31-25-112_rnn_no1_abcdefghijklmno_20_3.zip
20190827130301-172-31-28-104_rnn_no2_abcd_10_3.zip
20190827125741-172-31-23-42_rnn_no2_abcdef_10_3.zip
...
20190827070645-172-31-27-60_rnn_no5_abcdefghijklmno_20_3.zip"""
    ...
    tasks = []
    for rnn, param in itertools.product(rnns, params):
        splitted = rnn.split("_")[2:]
        tail = "_".join(splitted)
        s3 = f"extract_{tail}"
        method = param[0]
        eqq_param = param[1]
        tasks.append({"method": method, "eqq_param": eqq_param, "s3": s3, "rnn": rnn, "args_RNN": "artificial"})
    return tasks


def make_tasks_ADFA():
    tasks = []
    for param, top_n in itertools.product(make_params(), [151, 10, 20]):
        argsrnn = f"adfa_3_{top_n}.zip"
        method = param[0]
        eqq_param = param[1]
        tasks.append({"method": method, "eqq_param": eqq_param,
                      "s3": "extract_" + argsrnn, "rnn": "hoge", "args_RNN": argsrnn})
    return tasks


def make_tasks():
    return make_tasks_artificial()
    # return make_tasks_ADFA()


# Takes a task and return the result
def calc(d):
    with open("eqq_param.json", "w") as f:
        json.dump(d["eqq_param"], f)
    z = rnn2wfa.run(args_s3rnn=d["rnn"],
                    args_RNN=d["args_RNN"],
                    args_tol_rank_init=rnn2wfa.default_tol_rank_init,
                    args_tol_rank_decay_rate=0.9,
                    args_tol_rank_lower_bound=rnn2wfa.default_tol_rank_lower_bound,
                    args_qot_timeout=rnn2wfa.default_qot_timeout,
                    args_eqq_type=d["method"],
                    args_eqq_param="eqq_param.json",
                    args_save_process=True,
                    args_timeout=10000,
                    args_max_length=rnn2wfa.default_max_length,
                    args_s3=d["s3"])
